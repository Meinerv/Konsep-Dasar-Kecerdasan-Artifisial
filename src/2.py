...
def extract_features(image_path):
    ...
    return [r_avg, g_avg, b_avg,
            brightness, hue_mean, hue_std]

path_dataset = '/content/drive/MyDrive/KDKA/dataset_sampah'
...
class RandomForest:
    def __init__(self, n_estimators = 10, max_depth=7, min_samples_split=2):
...
print(f"\nWaktu Eksekusi Decision Tree: {run:.3f} detik.")

start_time = time.perf_counter()
clf_rf = RandomForest(n_estimators=23, max_depth=7)
clf_rf.fit(X_train, y_train)
end_time = time.perf_counter()

run = end_time - start_time
print(f"Waktu Eksekusi Random Forest: {run:.3f} detik.\n")
